...
export function Stars() {
  ...
  const [data, setData] = useState<TDataStars>({} as TDataStars)
  // const [data, setData] = useState()
  let randomNumber = 1 + Math.random() * (1000 - 1)
  let randomPick = 1 + Math.random() * (30 - 1)

  async function fetchAPI() {  
    const headers = { 'X-Api-Key': 'WGf2pE2BPrILpn0Ba/rCcw==CctM1QWlDIzo90xR'}
    await fetch(`https://api.api-ninjas.com/v1/stars?max_distance_light_year=${Math.round(randomNumber)}`, { headers })
